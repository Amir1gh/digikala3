...
def order_details(request,pk):
    if request.user.is_authenticated:
        ...
        messages.success(request,"دسترسی به این صفحه امکان پذیز نمی باشد")
...
def signup_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request,("مشکلی در ثبت نام شما وجود داره"))
...
def update_password(request):
    if request.user.is_authenticated:
        ...
        messages.success(request,"باید اول لاگین کنی")
        return redirect("home")
